...
describe("Accordion", () => {
  it("기본값은 collapsed 로 렌더하고 콘텐츠를 aria-hidden 처리한다", () => {
    render(<Accordion title="제목">콘텐츠</Accordion>);
  ...
  it("uncontrolled: 클릭 시 펼침/접힘이 토글되고 아이콘 이름이 바뀐다", async () => {
    ...
    const { container } = render(
      <Accordion title="제목" defaultExpanded={false}>
        콘텐츠
      </Accordion>,
    );
    const header = screen.getByRole("button", { name: "제목" });
  ...
  it("className 을 루트에 병합한다", () => {
    const { container } = render(
      <Accordion title="제목" className="w-full">
        콘텐츠
      </Accordion>,
    );
  ...
  it("마크업에 hex 가 없다", () => {
    const { container } = render(
      <Accordion title="제목" defaultExpanded>
        콘텐츠
      </Accordion>,
    );
